...
public class QuickEdit_NaturalDisturbances_Panel extends JPanel {
	
	public QuickEdit_NaturalDisturbances_Panel(JTable table7a, Object[][] data7a, JTable table7b, Object[][] data7b) {
				...
				for (int i : selectedRow) {
					for (int j : selectedCol) {
						if (!formatedTextfield.getText().equals(".") && j >= 2) {	// Only apply the changes to selected cells in columns >= 2 (fall the percentage columns)
							if (!formatedTextfield.getText().isEmpty()) data7a[i][j] = Double.valueOf(formatedTextfield.getText());	// Only apply the changes to selected cells in column 2 "weight", do not allow null
						}
					}
				}
				
				table7a.setValueAt(data7a[table7a.convertRowIndexToModel(0)][table7a.convertColumnIndexToModel(0)], 0, 0);		// Just to activate firesTabelDatachanged() --> update_Percentage_column is auto triggered
				// Reselect the current selected cells
				table7a.clearSelection(); // To help trigger the row refresh: clear then add back the rows
				for (int i : selectedRow) { table7a.addRowSelectionInterval(table7a.convertRowIndexToView(i), table7a.convertRowIndexToView(i)); }
				for (int j : selectedCol) { table7a.addColumnSelectionInterval(table7a.convertColumnIndexToView(j), table7a.convertColumnIndexToView(j)); }
			}
		});		
				...
				for (int i : selectedRow) {
					for (int j : selectedCol) {
						if (!formatedTextfield_2.getText().equals(".") && j >= 2) {	// Only apply the changes to selected cells in columns >= 2 (all the percentage columns)
							if (!formatedTextfield_2.getText().isEmpty()) data7b[i][j] = Double.valueOf(formatedTextfield_2.getText());	// Only apply the changes to selected cells in column 2 "weight", do not allow null
						}
					}
				}
				
				table7b.setValueAt(data7b[table7b.convertRowIndexToModel(0)][table7b.convertColumnIndexToModel(0)], 0, 0);		// Just to activate firesTabelDatachanged() --> update_Percentage_column is auto triggered
				// Reselect the current selected cells
				table7b.clearSelection(); // To help trigger the row refresh: clear then add back the rows
				for (int i : selectedRow) { table7b.addRowSelectionInterval(table7b.convertRowIndexToView(i), table7b.convertRowIndexToView(i)); }
				for (int j : selectedCol) { table7b.addColumnSelectionInterval(table7b.convertColumnIndexToView(j), table7b.convertColumnIndexToView(j)); }
			}
		});		
