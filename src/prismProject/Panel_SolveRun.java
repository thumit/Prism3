...
//									// total_Periods + 1 because tR starts from 1 to total_Periods, ignore the 0			
			...
			Arrays.sort(vname);						
			vnamelist = null;	// Clear the lists to save memory
			double[] objvals = new double[nV];
			double[] vlb = new double[nV];			
			double[] vub = new double[nV];			
			IloNumVarType[] vtype = new IloNumVarType[nV];						
			...
			for (int j = 0; j < total_softConstraints; j++) {
				...
				vtype[new_var_index] = (IloNumVarType.Float);
				y[j] = new_var_index;
				nvars++;				
			}
			nV = nvars;									
