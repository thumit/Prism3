...
public class Get_Parameter_Information {
	...
	public static double get_total_value(String var_name, int rotation_age,
		...
		if (all_dynamicIdentifiers_columnIndexes.contains("NoIdentifier") && parameters_indexes_list.contains("NoParameter")) {	// This is the only case when we don't need to check yield table
			...
			List<String> yieldTable_Name_list = new ArrayList<String>() {{ for (Object i : yield_tables_names) add(i.toString());}};		// Convert Object array to String list
			...
			if (yieldTable_Name_list.contains(yield_table_name_to_find)) {		// If yield table name exists						
				...
				if (row_id_to_find < yield_tables_values[table_id_to_find].length) {
					...
					boolean all_dynamicIdentifiers_matched = areAllDynamicIdentifiersMatched(yield_tables_values, table_id_to_find, row_id_to_find, all_dynamicIdentifiers_columnIndexes, all_dynamicIdentifiers);								
					if (all_dynamicIdentifiers_matched) {
						...
						if (parameters_indexes_list.contains("NoParameter")) {			//Return 1 if NoParameter & all dynamic identifiers match
							...
							int action_type_YTindex = yield_tables_values[table_id_to_find][row_id_to_find].length - 1;		// 'action_type' is the last column of each yield table
							String current_YTrow_action = yield_tables_values[table_id_to_find][row_id_to_find][action_type_YTindex].toString();
							...
							if (action_type_list.contains(current_YTrow_action)) {
								...
								for (int j = 0; j < baseCost_yieldtables[baseCost_row].length; j++) {		//loop all yield table columns (or baseCost_yieldtables columns)
									...
									if (baseCost_yieldtables[baseCost_row][col] > 0) {		
										value_to_return = value_to_return + baseCost_yieldtables[baseCost_row][col] * Double.parseDouble(yield_tables_values[table_id_to_find][row_id_to_find][col].toString());		// then add base_cost * parameter
									}
								}
